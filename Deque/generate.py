"""
generate.py —— Deque 脚本代码生成器

用法:
    python generate.py <suffix>

示例:
    python generate.py student   → 生成 ds_deque_student_type.h
                                      ds_deque_student.h
                                      ds_deque_student.c

工作流程:
    1. 用户编辑 ds_deque_type.h，定义想要的元素类型
    2. 运行本脚本，传入后缀名
...
注意:
    Deque 有 MATCH 宏，无游标类型。
    static inline 函数: destroy_element  clone_element  match_element。
"""
...
def two_pass_replace(text, replacements):
    ...
    为什么需要占位符？
        ...
        比如 old="ds_deque_" 的 new="ds_deque_student_"
...
def generate(suffix):
    ...
    type_src   = read("ds_deque_type.h")
    head_src   = read("ds_deque.h")
    source_src = read("ds_deque.c")

    # ── 替换规则 ──
    # 格式: (原始文本, 替换后文本)
    # 顺序任意 —— 算法自动按长度排序，保证长串先于短串处理。
    rules = [
        ("#include \"ds_deque_type.h\"", f"#include \"ds_deque_{suffix}_type.h\""),
        ("#include \"ds_deque.h\"",      f"#include \"ds_deque_{suffix}.h\""),

        ("DS_DEQUE_TYPE_H",          f"DS_DEQUE_{SUFFIX}_TYPE_H"),
        ("DS_DEQUE_DESTROY_ELEMENT", f"DS_DEQUE_{SUFFIX}_DESTROY_ELEMENT"),
        ("DS_DEQUE_CLONE_ELEMENT",   f"DS_DEQUE_{SUFFIX}_CLONE_ELEMENT"),
        ("DS_DEQUE_MATCH_TYPE",      f"DS_DEQUE_{SUFFIX}_MATCH_TYPE"),
        ("DS_DEQUE_MATCH",           f"DS_DEQUE_{SUFFIX}_MATCH"),
        ("DS_DEQUE_TYPE",            f"DS_DEQUE_{SUFFIX}_TYPE"),
        ("DS_DEQUE_H",               f"DS_DEQUE_{SUFFIX}_H"),
        ...
        ("ds_deque_type",            f"ds_deque_{suffix}_type"),
        ("DS_Deque",                 f"DS_Deque_{Cap}"),

        ("ds_deque_",                f"ds_deque_{suffix}_"),
    ]
    ...
    write(f"ds_deque_{suffix}_type.h", new_type)
    write(f"ds_deque_{suffix}.h",      new_header)
    write(f"ds_deque_{suffix}.c",      new_source)
...
def main():
    ...
    print(f"  后缀: {suffix}")
    print(f"{'='*60}\n")
    print("[读取] ds_deque_type.h, ds_deque.h, ds_deque.c")

    generate(suffix)

    print(f"\n{'='*60}")
    print(f"  完成！在当前目录生成了 3 个文件：")
    print(f"    ds_deque_{suffix}_type.h")
    print(f"    ds_deque_{suffix}.h")
    print(f"    ds_deque_{suffix}.c")
    print(f"")
    print(f"  在 main.c 中使用：")
    print(f"    #include \"ds_deque_{suffix}.h\"")
    print(f"{'='*60}\n")
