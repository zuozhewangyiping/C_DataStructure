"""
generate.py —— Queue 脚本代码生成器

用法:
    python generate.py <suffix>

示例:
    python generate.py student   → 生成 ds_queue_student_type.h
                                      ds_queue_student.h
                                      ds_queue_student.c

工作流程:
    1. 用户编辑 ds_queue_type.h，定义想要的元素类型
    2. 运行本脚本，传入后缀名
...
注意:
    Queue 结构最简——无 MATCH 宏，无游标类型，无比较宏。
...
def two_pass_replace(text, replacements):
    ...
    为什么需要占位符？
        ...
        比如 old="ds_queue_" 的 new="ds_queue_student_"
...
def generate(suffix):
    ...
    type_src   = read("ds_queue_type.h")
    head_src   = read("ds_queue.h")
    source_src = read("ds_queue.c")

    # ── 替换规则 ──
    # 格式: (原始文本, 替换后文本)
    # Queue 结构最简：无 MATCH，无游标，无比较宏。
    rules = [
        ("#include \"ds_queue_type.h\"", f"#include \"ds_queue_{suffix}_type.h\""),
        ("#include \"ds_queue.h\"",      f"#include \"ds_queue_{suffix}.h\""),

        ("DS_QUEUE_TYPE_H",          f"DS_QUEUE_{SUFFIX}_TYPE_H"),
        ("DS_QUEUE_DESTROY_ELEMENT", f"DS_QUEUE_{SUFFIX}_DESTROY_ELEMENT"),
        ("DS_QUEUE_CLONE_ELEMENT",   f"DS_QUEUE_{SUFFIX}_CLONE_ELEMENT"),
        ("DS_QUEUE_TYPE",            f"DS_QUEUE_{SUFFIX}_TYPE"),
        ("DS_QUEUE_H",               f"DS_QUEUE_{SUFFIX}_H"),
        ...
        ("ds_queue_type",            f"ds_queue_{suffix}_type"),
        ("DS_Queue",                 f"DS_Queue_{Cap}"),

        ("ds_queue_",                f"ds_queue_{suffix}_"),
    ]
    ...
    write(f"ds_queue_{suffix}_type.h", new_type)
    write(f"ds_queue_{suffix}.h",      new_header)
    write(f"ds_queue_{suffix}.c",      new_source)
...
def main():
    ...
    print(f"  后缀: {suffix}")
    print(f"{'='*60}\n")
    print("[读取] ds_queue_type.h, ds_queue.h, ds_queue.c")

    generate(suffix)

    print(f"\n{'='*60}")
    print(f"  完成！在当前目录生成了 3 个文件：")
    print(f"    ds_queue_{suffix}_type.h")
    print(f"    ds_queue_{suffix}.h")
    print(f"    ds_queue_{suffix}.c")
    print(f"")
    print(f"  在 main.c 中使用：")
    print(f"    #include \"ds_queue_{suffix}.h\"")
    print(f"{'='*60}\n")
